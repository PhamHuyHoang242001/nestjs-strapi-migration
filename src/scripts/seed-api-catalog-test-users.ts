...
// One-off, idempotent seeder for THREE API Catalog test accounts (view / upload / approve).
...
//
// Run:  npx ts-node -r tsconfig-paths/register src/scripts/seed-api-catalog-test-users.ts
...
const ACCOUNTS: AccountSpec[] = [
  { username: 'api_viewer', email: 'api.viewer@vpbank.com.vn', fullName: 'API Viewer', roleName: 'API Viewer', codes: [] },
  { username: 'api_uploader', email: 'api.uploader@vpbank.com.vn', fullName: 'API Uploader', roleName: 'API Uploader', codes: ['api_upload'] },
  { username: 'api_approver', email: 'api.approver@vpbank.com.vn', fullName: 'API Approver', roleName: 'API Approver', codes: ['api_upload', 'api_approve'] },
];
  ...
  try {
    await ds.transaction(async (m) => {
      ...
      const permRows = (await m.query(
        `SELECT id, code FROM ${permTable} WHERE code IN ('api_upload','api_approve') AND is_active = true AND deleted_at IS NULL`,
      )) as Array<{ id: number; code: string }>;
      const permId: Record<string, number> = {};
      for (const r of permRows) permId[r.code] = Number(r.id);
      for (const code of ['api_upload', 'api_approve']) {
        if (!permId[code]) throw new Error(`Permission '${code}' not found in ${permTable} — run the api-catalog migrations first.`);
      }
