...
// One-off, idempotent seeder for THREE Skill Package test accounts (view / upload / approve).
...
// name, role-permission grants via ON CONFLICT. New users need no Redis cache invalidation
// (per-user permission cache is populated lazily with a TTL on first check).
//
// Run:  npx ts-node -r tsconfig-paths/register src/scripts/seed-skill-test-users.ts
...
interface AccountSpec {
  ...
  fullName: string;
  roleName: string;
  codes: string[]; // skill permission codes granted to this account's role
}

const ACCOUNTS: AccountSpec[] = [
  { username: 'skill_viewer', email: 'skill.viewer@vpbank.com.vn', fullName: 'Skill Viewer', roleName: 'Skill Viewer', codes: [] },
  { username: 'skill_uploader', email: 'skill.uploader@vpbank.com.vn', fullName: 'Skill Uploader', roleName: 'Skill Uploader', codes: ['skill_upload'] },
  { username: 'skill_approver', email: 'skill.approver@vpbank.com.vn', fullName: 'Skill Approver', roleName: 'Skill Approver', codes: ['skill_upload', 'skill_approve'] },
];
...
async function main(): Promise<void> {
  ...
  try {
    await ds.transaction(async (m) => {
      ...
      // Resolve skill permission ids by code.
      const permRows = (await m.query(
        `SELECT id, code FROM ${permTable} WHERE code IN ('skill_upload','skill_approve') AND is_active = true AND deleted_at IS NULL`,
      )) as Array<{ id: number; code: string }>;
      const permId: Record<string, number> = {};
      for (const r of permRows) permId[r.code] = Number(r.id);
      for (const code of ['skill_upload', 'skill_approve']) {
        if (!permId[code]) throw new Error(`Permission '${code}' not found in ${permTable} — run the skill-package migrations first.`);
      }
      ...
      for (const a of ACCOUNTS) {
        ...
        console.log(`  ✓ ${a.email.padEnd(34)} user#${userId} role#${roleId}  [${a.codes.join(', ') || 'view-only'}]`);
      }
    });
