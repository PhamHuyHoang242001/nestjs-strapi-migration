...
// Skill Package migration 2608111000): api_upload (id=117) and api_approve (id=118).
// IDs verified non-colliding against live DB (MAX after category perm 116 at authoring time 2026-08-12).
//
...
// the Skill Package feature shares it; only the two api_* permission rows are removed.

const MODULE_ID = 104;
const PERMISSION_ID_UPLOAD = 117;
const PERMISSION_ID_APPROVE = 118;
...
const EXPECTED_PERMISSIONS = new Map<number, string>([
  [PERMISSION_ID_UPLOAD, 'api_upload'],
  [PERMISSION_ID_APPROVE, 'api_approve'],
]);

export class AddApiCatalogPermissions1784990001000 implements MigrationInterface {
  name = 'AddApiCatalogPermissions1784990001000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      ...
      VALUES
        (${PERMISSION_ID_UPLOAD},  'Upload API',  'api_upload',  'POST',  'upload',  true, ${MODULE_ID}),
        (${PERMISSION_ID_APPROVE}, 'Approve API', 'api_approve', 'PATCH', 'approve', true, ${MODULE_ID})
      ON CONFLICT (id) DO UPDATE
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    // Remove ONLY the two api_* permissions. Module 104 (Asset Hub) is intentionally left in
