...
import { ListApiQueryDto } from './dto/list-api-query.dto';
...
export class ApiCatalogQueryService {
  ...
  async list(query: ListApiQueryDto, userId: number) {
    ...
    applyAssetHubCatalogFilters(qb, 'api-catalog', query);
    if (query.http_method) {
      qb.andWhere('av.http_method = :http_method', { http_method: query.http_method });
    }
  ...
  async listVersions(query: ListVersionsDto, userId: number) {
    ...
    if (query.api_catalog_package_id?.length) {
      params.push(query.api_catalog_package_id);
    ...
    const data = rows.map((r) => ({
      ...
      is_first_pending: r.state === ApiVersionState.PENDING && r.old_version == null,
  ...
  async listReviews(query: ReviewQueryDto) {
    ...
    const qb = this.versionRepo
      ...
      .andWhere('pv.state = :state', { state: ApiVersionState.PENDING });
  ...
  async versionDetail(versionId: number, userId: number) {
    ...
    if (version.state === ApiVersionState.PENDING) {
      let predecessor: ApiVersion | null = null;
      if (version.old_version != null) {
        predecessor = await this.versionRepo.findOne({
          where: {
            api_catalog_package_id: version.api_catalog_package_id,
      ...
      comparison = {
        ...
        base: predecessor ? stringifyApiSpec(predecessor) : null,
  ...
  async getDiff(versionId: number, userId: number) {
    ...
    if (pkg?.active_version_id && pkg.active_version_id !== versionId) {
      ...
      baseContent = activeVersion ? stringifyApiSpec(activeVersion) : null;
    }
    ...
    return {
      ...
      metadata: {
        ...
        // Stable public code of the item (api_<id>). Additive: lets the review UI show the same
        ...
        tags: (await this.metaRead.getTagsByVersionIds('api-catalog', [version.id])).get(version.id) ?? [],
  ...
  async resolveActiveForExport(
    ...
    if (!pkg.active_version_id || !pkg.active_version || pkg.active_version.is_deleted) {
      throw new NotFoundException('API package has no active version');
    }
    ...
    const tags = (await this.metaRead.getTagsByVersionIds('api-catalog', [pkg.active_version.id])).get(
