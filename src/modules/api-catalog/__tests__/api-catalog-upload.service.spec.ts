...
describe('ApiCatalogUploadService', () => {
  let service: ApiCatalogUploadService;
  let packageRepo: any;
  let versionRepo: any;
  let dataSource: any;
  let avatarUrl: any;
  ...
  describe('createNew — persists avatar_url inline; no file rows', () => {
    const dto = {
    ...
    it('first version is old_version=null, version_no=1, and sets code=api_<id> post-insert in the tx', async () => {
      const saved: any[] = [];
      const updates: any[] = [];
      ...
      expect(versionRow.state).toBe(ApiVersionState.PENDING);
      const codeUpdate = updates.find((u) => u.entity === 'ApiPackage');
      expect(codeUpdate).toBeDefined();
      expect(codeUpdate.patch).toEqual({ code: 'api_catalog_1' });
    });

    it('stores avatar_url inline on the v1 version; no file entity saved', async () => {
      const saved: any[] = [];
      ...
      expect(versionRow).not.toHaveProperty('definition');
      // No ZIP/file concept exists — nothing but ApiPackage + ApiVersion is saved.
      expect(saved.some((s) => s.entity === 'ApiVersionFile')).toBe(false);
  ...
  describe('approve — single tx sets active_version_id and version.state', () => {
    it('approve sets state=approved and package.active_version_id=vid in same tx', async () => {
      const version = { id: VERSION_ID, api_catalog_package_id: PACKAGE_ID, state: ApiVersionState.PENDING };
      const pkg = { id: PACKAGE_ID, active_version_id: null, status: ApiPackageStatus.ACTIVE };
      ...
      expect(savedVersion?.state).toBe(ApiVersionState.APPROVED);
    ...
    it('finalizes version_no=(old_version ?? 0)+1: update-pending old_version=1 → approved version_no=2', async () => {
      ...
      const pkg = { id: PACKAGE_ID, active_version_id: 3, status: ApiPackageStatus.ACTIVE };
      let savedVersion: any;
      dataSource.transaction = jest.fn(async (cb: any) => {
        const manager = {
          ...
          save: jest.fn(async (_E: any, obj: any) => {
            if (obj.version_no !== undefined && obj.state === ApiVersionState.APPROVED) savedVersion = obj;
            return { ...obj };
          }),
        };
        return cb(manager);
    ...
    it('first approve (old_version=null) → version_no=1', async () => {
      const version = { id: VERSION_ID, api_catalog_package_id: PACKAGE_ID, state: ApiVersionState.PENDING, old_version: null, version_no: 1 };
      const pkg = { id: PACKAGE_ID, active_version_id: null, status: ApiPackageStatus.ACTIVE };
      let savedVersion: any;
      dataSource.transaction = jest.fn(async (cb: any) => {
        const manager = {
    ...
    it('approve of non-pending version → ForbiddenException', async () => {
      dataSource.transaction = jest.fn(async (cb: any) => {
        const manager = {
          findOne: jest.fn().mockResolvedValue({ id: VERSION_ID, state: ApiVersionState.APPROVED }),
          save: jest.fn(),
        };
        return cb(manager);
      });
  ...
  describe('reject — reason is required', () => {
    it('reject with reason → state=rejected + reject_reason saved', async () => {
      ...
      expect(saved.state).toBe(ApiVersionState.REJECTED);
  ...
  describe('createVersion — success path + pending guard', () => {
    ...
    it('sets old_version=latest approved, version_no=placeholder (=old_version) inline in one tx', async () => {
      ...
      const versionRow = saved.find((s) => s.entity === 'ApiVersion')?.row;
    ...
    it('existing pending version → ConflictException before avatar validation or transaction', async () => {
      packageRepo.findOne = jest.fn().mockResolvedValue({ id: PACKAGE_ID, is_deleted: false, created_by: USER_ID });
      permissionQuery.getUserPermissions.mockResolvedValue(['api_upload']);
      versionRepo.findOne.mockResolvedValue({ id: VERSION_ID, state: ApiVersionState.PENDING });
    ...
    it('non-owner non-approver → ForbiddenException (guard before avatar validation)', async () => {
      ...
      permissionQuery.getUserPermissions.mockResolvedValue(['api_upload']); // no api_approve
    ...
    it('owner with api_upload (no approve) → allowed', async () => {
      packageRepo.findOne = jest.fn().mockResolvedValue({ id: PACKAGE_ID, is_deleted: false, created_by: USER_ID });
      permissionQuery.getUserPermissions.mockResolvedValue(['api_upload']);
    ...
    it('owner + approver → allowed (both conditions satisfied)', async () => {
      packageRepo.findOne = jest.fn().mockResolvedValue({ id: PACKAGE_ID, is_deleted: false, created_by: USER_ID });
      permissionQuery.getUserPermissions.mockResolvedValue(['api_upload', 'api_approve']);
  ...
  describe('toggleStatus — active/inactive visibility toggle', () => {
    it('updates the package status and returns {id, status}', async () => {
      ...
      expect(result).toEqual({ id: PACKAGE_ID, status: ApiPackageStatus.INACTIVE });
    });
