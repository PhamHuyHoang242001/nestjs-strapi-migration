...
function makeHarness(overrides: { itemMeta?: Record<string, jest.Mock> } = {}) {
    ...
    transaction: jest.fn(async (cb: (m: unknown) => unknown) => {
      const manager = {
        ...
        save: jest.fn(async (_E: { name?: string }, obj: Record<string, unknown>) => {
          ...
          if (_E?.name === 'PromptVersion') order.push('version');
          if (_E?.name === 'PromptPackage') order.push('package');
          return row;
        }),
  ...
  const permissionQuery = { getUserPermissions: jest.fn().mockResolvedValue(['prompt_upload']) };

  const service = new PromptLibraryUploadService(
...
const versionRow = (saved: Array<{ entity?: string; row: Record<string, unknown> }>) =>
  saved.find((s) => s.entity === 'PromptVersion')?.row;

describe('PromptLibraryUploadService — asset-hub metadata on create', () => {
  it('persists publisher_id on the package', async () => {
    const h = makeHarness();

    await h.service.createNew(baseCreateDto as never, USER_ID);

    expect(h.saved.find((s) => s.entity === 'PromptPackage')?.row.publisher_id).toBe(PUBLISHER_ID);
  });
  ...
  it('validates tags against the prompt workspace', async () => {
    ...
    expect(h.itemMeta.assertTags).toHaveBeenCalledWith(expect.anything(), [21], 'prompt');
  });
  ...
  it('writes the people in charge and the version tags in the create transaction', async () => {
    ...
    expect(h.itemMeta.replaceResponsibles).toHaveBeenCalledWith(expect.anything(), 'prompt', 1, [11, 12]);
    expect(h.itemMeta.replaceVersionTags).toHaveBeenCalledWith(expect.anything(), 'prompt', 1, [21]);
  ...
  it('surfaces an unknown person in charge as a 400 and writes nothing', async () => {
    ...
    expect(h.saved.some((s) => s.entity === 'PromptVersion')).toBe(false);
  });
...
describe('PromptLibraryUploadService — asset-hub metadata on bump', () => {
  it('updates the package publisher inside the version transaction', async () => {
    ...
    expect(h.updates).toContainEqual({ entity: 'PromptPackage', id: PACKAGE_ID, patch: { publisher_id: 9 } });
  ...
  it('replaces the people in charge on a bump', async () => {
    ...
    expect(h.itemMeta.replaceResponsibles).toHaveBeenCalledWith(expect.anything(), 'prompt', PACKAGE_ID, [55]);
  });
...
describe('prompt write DTOs', () => {
  it('rejects a create without a publisher', async () => {
    const errors = await validate(plainToInstance(CreatePromptPackageDto, { ...baseCreateDto, publisher_id: undefined }));
  ...
  it('rejects a create with no person in charge', async () => {
    const errors = await validate(
      plainToInstance(CreatePromptPackageDto, { ...baseCreateDto, responsible_user_ids: [] }),
    );
  ...
  it('accepts a fully populated create body', async () => {
    const errors = await validate(plainToInstance(CreatePromptPackageDto, baseCreateDto));

    expect(errors).toHaveLength(0);
  });

  it('accepts a bump body with an empty guide', async () => {
    const errors = await validate(plainToInstance(CreatePromptVersionDto, { ...baseVersionDto, usage_guide_html: '' }));
  ...
  it.each([
    ['create', CreatePromptPackageDto, baseCreateDto],
    ['bump', CreatePromptVersionDto, baseVersionDto],
...
describe('PromptLibraryController — no package metadata endpoint', () => {
  it('exposes the status toggle but no bare item PATCH', () => {
    const proto = PromptLibraryController.prototype as unknown as Record<string, unknown>;
