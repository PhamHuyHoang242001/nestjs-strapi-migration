...
describe('PromptLibraryQueryService', () => {
  ...
  beforeEach(() => {
    ...
    service = new PromptLibraryQueryService(packageRepo, versionRepo, permissionQuery, metaRead);
  });
  ...
  describe('list — active-only filter + sort + limit cap', () => {
    ...
    it('search also matches a catalog tag NAME, via a non-correlated subquery', async () => {
      ...
      expect(joined).toContain('SELECT vt.prompt_version_id FROM prompt_version_tags vt');
  ...
  describe('listVersions — visibility from permission set (C1), thin projection, filters', () => {
    ...
    it('isUpdate=true only for latest rejected with no pending, submitter, and prompt_upload', async () => {
      permissionQuery.getUserPermissions.mockResolvedValue(['prompt_upload']);
      const query = stub({
        count: 2,
        rows: [
          rawRow({ version_id: 10, state: PromptVersionState.REJECTED, is_update: true }),
          rawRow({ version_id: 9, state: PromptVersionState.APPROVED, is_update: false }),
        ],
      });
    ...
    it('isUpdate=false when caller lacks prompt_upload even if SQL marks the row editable', async () => {
      permissionQuery.getUserPermissions.mockResolvedValue(['prompt_approve']);
      stub({ count: 1, rows: [rawRow({ state: PromptVersionState.REJECTED, is_update: true })] });
