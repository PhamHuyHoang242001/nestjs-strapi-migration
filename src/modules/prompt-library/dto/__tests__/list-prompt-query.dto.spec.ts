...
import { ListPromptQueryDto } from '../list-prompt-query.dto';
...
describe('ListPromptQueryDto — list has no tag_ids / kind', () => {
  const parse = async (raw: Record<string, unknown>) => {
    const dto = plainToInstance(ListPromptQueryDto, raw, { enableImplicitConversion: false });
