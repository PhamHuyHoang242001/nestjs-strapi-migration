...
describe('prompt ListVersionsDto — prompt_package_id transform (comma string → number[])', () => {
  ...
  it('comma string "1,2,3" → [1,2,3]', async () => {
    const { dto, errors } = await parse({ prompt_package_id: '1,2,3' });
    expect(dto.prompt_package_id).toEqual([1, 2, 3]);
    expect(errors).toHaveLength(0);
  });

  it('trims whitespace: " 1 , 2 ,3 " → [1,2,3]', async () => {
    const { dto } = await parse({ prompt_package_id: ' 1 , 2 ,3 ' });
    expect(dto.prompt_package_id).toEqual([1, 2, 3]);
  });

  it('single value "5" → [5]', async () => {
    const { dto, errors } = await parse({ prompt_package_id: '5' });
    expect(dto.prompt_package_id).toEqual([5]);
    expect(errors).toHaveLength(0);
  });

  it('repeated/bracket-array form ["1","2"] → [1,2]', async () => {
    const { dto, errors } = await parse({ prompt_package_id: ['1', '2'] });
    expect(dto.prompt_package_id).toEqual([1, 2]);
    expect(errors).toHaveLength(0);
  });

  it('rejects mixed non-numeric and non-positive tokens', async () => {
    const { dto, errors } = await parse({ prompt_package_id: '1,abc,-3,0,4' });
    expect(dto.prompt_package_id).toEqual([1, NaN, -3, 0, 4]);
    expect(errors.length).toBeGreaterThan(0);
  });

  it('rejects an all-junk filter instead of widening to an unfiltered query', async () => {
    const { dto, errors } = await parse({ prompt_package_id: 'abc,xyz' });
    expect(dto.prompt_package_id).toEqual([NaN, NaN]);
    expect(errors.length).toBeGreaterThan(0);
  });

  it('omitted → undefined (no filter)', async () => {
    const { dto, errors } = await parse({ state: 'all' });
    expect(dto.prompt_package_id).toBeUndefined();
    expect(errors).toHaveLength(0);
  });

  it('empty string → undefined (no filter)', async () => {
    const { dto } = await parse({ prompt_package_id: '' });
    expect(dto.prompt_package_id).toBeUndefined();
  });
});
