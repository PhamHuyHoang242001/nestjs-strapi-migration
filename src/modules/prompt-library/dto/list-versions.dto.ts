...
// Query DTO for GET /v1/prompt/versions — the flat 1-row-per-version management list.
// Visibility is derived server-side from the caller's permission set (NOT from any field here):
// prompt_package_id is ONLY a display filter, never an authorization input.
export class ListVersionsDto {
  // Filter to specific prompt packages. Sent as a comma-separated id list (e.g. "1,2,3"); the service
  ...
  @ApiProperty({ required: false, description: 'Filter to these prompt package ids, comma-separated (e.g. "1,2,3")', type: String })
  @IsOptional()
  @IsArray()
  @IsInt({ each: true })
  @Min(1, { each: true })
  ...
  readonly prompt_package_id?: number[];
