...
export class ChangeHistoryRepository extends BaseRepository<ChangeHistory> {
  ...
  buildSearchQuery(dto: SearchChangeHistoryDto, sortParams: SortParams) {
    ...
    if (dto.performed_by) query.andWhere('ch.performed_by ILIKE :pb', { pb: `%${dto.performed_by}%` });
    ...
    if (dto.search)
      query.andWhere('(ch.entity_name ILIKE :s OR CAST(ch.entity_id AS TEXT) ILIKE :s OR ch.performed_by ILIKE :s)', {
        s: `%${dto.search}%`,
      });
