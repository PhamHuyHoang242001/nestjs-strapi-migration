  ...
  describe('listVersions — visibility from permission set (C1), thin projection, filters', () => {
    ...
    it('isUpdate=true only for rejected newest live version, submitter, and skill_upload', async () => {
      ...
      const query = stub({
        count: 2,
        rows: [
          rawRow({ version_id: 10, state: SkillVersionState.REJECTED, is_update: true }),
          rawRow({ version_id: 9, state: SkillVersionState.APPROVED, is_update: false }),
        ],
      });
    ...
    it('isUpdate=false when caller lacks skill_upload even if SQL marks the row editable', async () => {
      permissionQuery.getUserPermissions.mockResolvedValue(['skill_approve']);
      stub({ count: 1, rows: [rawRow({ state: SkillVersionState.REJECTED, is_update: true })] });
